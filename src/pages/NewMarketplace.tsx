...
const REGISTRY_URL =
  import.meta.env['VITE_SERVER_URL'] || 'https://apps.calimero.network';
...
export default function NewMarketplace() {
      ...
      apps.forEach((a) => {
        ...
        try {
          const meta = JSON.parse(
            new TextDecoder().decode(new Uint8Array(a.metadata || [])),
          );
  ...
  const loadMarketplace = useCallback(async () => {
    ...
    try {
      ...
      const list: RegistryBundle[] = Array.isArray(data)
        ? data
        : data.bundles || [];
      setBundles(list);
    } catch (e: any) {
  ...
  const handleInstall = async (bundle: RegistryBundle) => {
    ...
    try {
      ...
      const manifestRes = await fetch(
        `${REGISTRY_URL}/api/v2/bundles/${encodeURIComponent(bundle.package)}/${encodeURIComponent(bundle.appVersion)}`,
      );
      if (!manifestRes.ok)
      ...
      if (manifest.artifact) {
        ...
        // V2 format
        const mpkArtifact = manifest.artifacts.find(
          (a: any) => a.type === 'mpk',
        );
        const wasmArtifact = manifest.artifacts.find(
          (a: any) => a.type === 'wasm',
        );
        if (mpkArtifact) {
          wasmUrl =
            mpkArtifact.mirrors?.[0] ||
            `https://ipfs.io/ipfs/${mpkArtifact.cid}`;
          wasmHashHex = mpkArtifact.sha256?.replace('sha256:', '') || null;
          if (
            !wasmHashHex &&
            mpkArtifact.cid &&
            /^[0-9a-f]{64}$/i.test(mpkArtifact.cid)
          ) {
            wasmHashHex = mpkArtifact.cid;
          }
        } else if (wasmArtifact) {
          wasmUrl =
            wasmArtifact.mirrors?.[0] ||
            `https://ipfs.io/ipfs/${wasmArtifact.cid}`;
          wasmHashHex = wasmArtifact.sha256?.replace('sha256:', '') || null;
          if (
            !wasmHashHex &&
            wasmArtifact.cid &&
            /^[0-9a-f]{64}$/i.test(wasmArtifact.cid)
          ) {
      ...
      const metadataJson = JSON.stringify({
        name,
        description:
          manifest.metadata?.description || bundle.metadata?.description || '',
      ...
      const metadataBytes = wasmUrl.endsWith('.mpk')
        ? new Uint8Array(0)
        : new TextEncoder().encode(metadataJson);

      const res = await apiClient
        .node()
        .installApplication(wasmUrl, metadataBytes, wasmHashBase58);
      if (res.error)
        throw new Error(res.error.message?.slice(0, 120) || 'Install failed');
                ...
                <div key={key} className="mkt-card">
                  <div className="mkt-card-header">
                    ...
                    <div className="mkt-card-title-section">
                      ...
                      <span className="mkt-card-version">
                        v{bundle.appVersion}
                      </span>
                    </div>
                    {isInstalled && (
                      <CheckCircleIcon className="mkt-installed-icon" />
                    )}
                  </div>
                  ...
                  <div className="mkt-card-meta">
                    <span className="mkt-meta-item" title={bundle.package}>
                      {bundle.package}
                    </span>
                    {author && (
                      <span className="mkt-meta-item">by {author}</span>
                    )}
                    {bundle.downloads != null && bundle.downloads > 0 && (
                      <span className="mkt-meta-item mkt-downloads">
