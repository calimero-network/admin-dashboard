...
export default function BlobsPage() {
      ...
      await Promise.all(
        batch.map(async (blob) => {
          try {
            ...
            console.warn(`Failed to detect type for blob ${blob.blob_id}:`, error);
  ...
  const downloadBlob = async (blobId: string) => {
    try {
      ...
      setErrorMessage(`Failed to download blob: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const detectFileType = async (blob: Blob, blobId: string): Promise<{ filename: string; mimeType: string }> => {
    ...
    const signatures = [
      ...
      { pattern: /^377abcaf271c/, ext: '7z', mime: 'application/x-7z-compressed' },
    ...
    if (isValidText && uint8Array.length > 0) {
      ...
      if (textContent.includes('function') || textContent.includes('const ') || textContent.includes('var ')) {
