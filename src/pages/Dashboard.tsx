  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        installedApps = ((res.data as any)?.data?.apps ?? (res.data as any)?.apps ?? []).length;
      } catch {}
      ...
      try {
        ...
        contexts = ((res.data as any)?.data?.contexts ?? (res.data as any)?.contexts ?? []).length;
      } catch {}
    ...
    <div className="app-shell">
      ...
      <main className="page-content">
        ...
        <div className={`dash-status-banner ${stats.nodeStatus}`}>
          ...
          <span className="dash-status-url">{getAppEndpointKey() || 'Not configured'}</span>
        </div>

        {/* Stats grid */}
        <div className="dash-stats-grid">
          <div className="dash-stat-card" onClick={() => navigate('/applications')}>
          ...
          <div className="dash-stat-card" onClick={() => navigate('/marketplace')}>
        ...
        <div className="dash-section">
          ...
          <div className="dash-actions-grid">
            <button className="dash-action-card" onClick={() => navigate('/marketplace')}>
              <ShoppingBagIcon className="dash-action-icon" />
              <span className="dash-action-label">Browse Marketplace</span>
              <span className="dash-action-desc">Install apps from the registry</span>
            </button>
            <button className="dash-action-card" onClick={() => navigate('/contexts')}>
              <CubeTransparentIcon className="dash-action-icon" />
              <span className="dash-action-label">Manage Contexts</span>
              <span className="dash-action-desc">Create or delete app contexts</span>
            </button>
            <button className="dash-action-card" onClick={() => navigate('/identity')}>
              <KeyIcon className="dash-action-icon" />
              <span className="dash-action-label">Identity & Keys</span>
              <span className="dash-action-desc">Manage root and client keys</span>
            </button>
            <button className="dash-action-card" onClick={() => navigate('/applications')}>
              <Square2StackIcon className="dash-action-icon" />
              <span className="dash-action-label">Applications</span>
              <span className="dash-action-desc">Manage installed applications</span>
            </button>
          </div>
        </div>
        ...
        <div className="dash-section">
          ...
          <div className="dash-actions-grid">
            {([
              { icon: GlobeAltIcon, label: 'Website', desc: 'calimero.network', href: 'https://calimero.network' },
              { icon: ArrowDownTrayIcon, label: 'Download Desktop', desc: 'Get the desktop app', href: 'https://calimero.network/download' },
              { icon: ShoppingBagIcon, label: 'App Registry', desc: 'Browse all apps', href: 'https://apps.calimero.network' },
              { icon: BookOpenIcon, label: 'Documentation', desc: 'Guides and API reference', href: 'https://docs.calimero.network' },
              { icon: CodeBracketSquareIcon, label: 'GitHub', desc: 'Open source repositories', href: 'https://github.com/calimero-network' },
